...
const LeaveApproval = () => {
    ...
    const fetchLeaveRequests = async () => {
        try {
            const response = await axios.get(`http://192.168.0.104:5000/api/leave-requests?role=${role}`);
    ...
    const handleStatusChange = async (id, newStatus) => {
        try {
            const response = await axios.put(`http://192.168.0.104:5000/api/leave-requests/${id}?role=${role}`, { status: newStatus });

        ...
        const fetchUsersAndStatus = async () => {
            try {
                ...
                const [usersResponse, statusResponse] = await Promise.all([
                    axios.get('http://192.168.0.104:5000/api/users'),
                    axios.get('http://192.168.0.104:5000/api/users/status')
                ]);
                        ...
                        {filteredUsers.length > 0 ? filteredUsers.map((user) => (
                            <tr key={user.id} className="align-middle">
                                <td style={{ width: '100px' }}>
                                    <img
                                        src={`http://192.168.0.104:5000${user.images}`}
                                        alt={user.name}
                                        width="70"
                                        className="me-3"
                        ...
                        >
                            <div className="modal-content">
                                ...
                                <div className="modal-body d-flex">
                                    <img src={`http://192.168.0.104:5000${selectedUser.images}`} alt={selectedUser.name} style={{ width: '150px', height: '150px', objectFit: 'cover', marginRight: '20px' }} />
                                    <div>
